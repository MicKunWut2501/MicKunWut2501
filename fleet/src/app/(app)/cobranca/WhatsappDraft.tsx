...
export function WhatsappDraft({ driverId, driverName, weekStart, phone, initial }: { driverId: string; driverName: string; weekStart: string; phone: string | null; initial: string }) {
  ...
  function send() {
    start(async () => {
      ...
      setStatus("Registado. A abrir o WhatsApp…");
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
      <div className="mb-2 flex items-center justify-between text-sm">
        ...
        <span className="text-xs text-gray-500">{phone ?? "sem telefone registado"}</span>
      </div>
      ...
      <div className="mt-2 flex items-center gap-3">
        <Button onClick={send} disabled={pending || message.trim().length < 10}>{pending ? "…" : "Enviar via WhatsApp"}</Button>
        <Button variant="ghost" type="button" onClick={() => setMessage(initial)}>Repor texto</Button>
        {status && <span className="text-xs text-gray-600">{status}</span>}
      </div>
      <p className="mt-1 text-xs text-gray-500">Nada é enviado sem o seu clique. Cada envio fica registado.</p>
    </div>
  );
}
