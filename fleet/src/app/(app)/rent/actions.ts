...
export async function recordPayment(_prev: ActionResult | undefined, formData: FormData): Promise<ActionResult> {
  ...
  if (!parsed.success) return { ok: false, error: "Invalid data: check the amount and date." };
  const p = parsed.data;
  if (weekStartOf(p.week_start) !== p.week_start) return { ok: false, error: "The week must start on a Monday." };
