...
export default async function VehiclePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <PageHeader title={vehicle.plate} subtitle={`${vehicle.model} · em serviço desde ${formatDate(vehicle.in_service_from)} · ${formatKm(vehicle.odometer_km)}`} actions={<Link href="/recibos/novo" className="rounded-lg bg-gray-900 px-3 py-1.5 text-sm font-medium text-white">Registar despesa</Link>} />

      <div className="grid gap-4 sm:grid-cols-3">
        <Stat label="Custo total registado" value={formatAOA(total)} sub={`${events.length} evento(s)`} />
        <Stat label="Custo por km" value={cpk ? `${formatAOA(cpk.per_km, { decimals: 1 })}/km` : "—"} sub={cpk ? `${formatKm(cpk.km)} entre leituras` : "precisa de 2 leituras de odómetro"} />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card title="Custo mensal por categoria">
          {chartRows.length ? <CategoryStackChart rows={chartRows} categories={usedCats} labels={CATEGORY_LABELS} /> : <Empty>Sem despesas registadas.</Empty>}
        </Card>
        <Card title="Tabela mensal">
          {months.length ? (
            <Table>
              <thead><tr><th className={th}>Mês</th>{usedCats.map((c) => <th key={c} className={thNum}>{CATEGORY_LABELS[c]}</th>)}<th className={thNum}>Total</th></tr></thead>
              <tbody>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2" title="Cronologia de eventos">
          {events.length ? (
          ...
          ) : <Empty>Sem eventos.</Empty>}
        </Card>
        <div className="space-y-4">
          <Card title="Motoristas">
            <ul className="space-y-1 text-sm">
              {schedules.filter((s) => s.vehicle_id === id).map((s) => (<li key={s.id}>{driverName.get(s.driver_id)} · {formatAOA(s.weekly_rent_aoa)}/sem · {formatDate(s.valid_from)} → {s.valid_to ? formatDate(s.valid_to) : "actual"}</li>))}
            </ul>
          </Card>
          <Card title="Paragens (sem renda esperada)">
            {downtime.length ? <ul className="space-y-1 text-sm">{downtime.map((d) => <li key={d.id}>{formatDate(d.from_date)} → {formatDate(d.to_date)} · {d.reason}</li>)}</ul> : <Empty>Sem paragens registadas.</Empty>}
