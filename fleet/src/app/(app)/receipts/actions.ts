...
export async function saveReceiptEvent(_prev: SaveResult | undefined, formData: FormData): Promise<SaveResult> {
  ...
  if (!isStaff(session)) return { ok: false, error: "Only the owner or an admin can save expenses." };
  const parsed = EventSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) return { ok: false, error: "Check vehicle, date, category and total." };
  const p = parsed.data;
  ...
  if (file instanceof File && file.size > 0) {
    if (file.size > MAX_BYTES) return { ok: false, error: "File larger than 10 MB." };
    if (!ALLOWED.has(file.type)) return { ok: false, error: "Unsupported format (JPG, PNG, WEBP, GIF or PDF)." };
  ...
  revalidatePath(`/fleet/${p.vehicle_id}`);
  revalidatePath("/receipts");
  revalidatePath("/dashboard");
  return { ok: true, message: "Expense saved.", eventId: ev.id as string };
}
