...
export default async function DriverPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const subtitle = `${driver.phone_e164 ?? "no phone"} · ${driver.active ? "active" : "inactive"} · ${assignments || "no assignment"}`;
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat label="Score" value={me?.score === null || me?.score === undefined ? "—" : String(me.score)} sub={me ? (me.score === null ? <Badge tone="warn">insufficient data</Badge> : <TrendArrow trend={me.trend} previous={me.previous_score} />) : undefined} />
        <Stat label="Top positive factor" value={me?.top_positive?.label ?? "—"} sub={me?.top_positive?.raw_label} />
        <Stat label="Top negative factor" value={me?.top_negative?.label ?? "—"} sub={me?.top_negative?.raw_label} />
      </div>

      <Card className="mt-6" title={`Rent history · ${formatWeek(window.from)} → ${formatWeek(window.to)}`}>
        {weeks.length ? (
          <>
            <div className="flex flex-wrap gap-1">
              {weeks.map((w) => (
                <div key={w.week_start} title={`${formatWeek(w.week_start)}: ${w.status} · paid ${formatAOA(w.paid_aoa)} of ${formatAOA(w.expected_aoa)}`} className={`h-8 w-8 rounded ${HEAT[w.status]}`} />
              ))}
            </div>
            <Table className="mt-3">
              <thead><tr><th className={th}>Week</th><th className={th}>Vehicle</th><th className={th}>Status</th><th className={thNum}>Expected</th><th className={thNum}>Paid</th><th className={thNum}>Outstanding</th></tr></thead>
        ...
        ) : <Empty>No rent weeks in the window.</Empty>}
      </Card>

      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Card title="Cost incidents (repair, brakes, tyres, fines)">
          {incidents.length ? <ul className="space-y-1 text-sm">{incidents.map((i) => <li key={i.event_id}>{formatDate(i.event_date)} · {plate.get(i.vehicle_id)} · {CATEGORY_LABELS[i.category]} · <span className="font-medium">{formatAOA(i.total_aoa)}</span>{i.notes && <span className="text-gray-500"> · {i.notes}</span>}</li>)}</ul> : <Empty>No incidents.</Empty>}
        </Card>
        <Card title="Vehicle downtime">
          {downtime.length ? <ul className="space-y-1 text-sm">{downtime.map((d) => <li key={d.id}>{formatDate(d.from_date)} → {formatDate(d.to_date)} · {plate.get(d.vehicle_id)} · {d.reason}</li>)}</ul> : <Empty>No downtime.</Empty>}
        </Card>
        <Card title="Payments and messages">
          <ul className="space-y-1 text-sm">
            ...
            {!payments.length && !log.length && <Empty>Nothing recorded.</Empty>}
          </ul>
        </Card>
      </div>
    </>
